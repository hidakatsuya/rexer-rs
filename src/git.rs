...
impl GitManager {
    ...
    fn checkout_reference(repo_path: &Path, reference: &str) -> Result<()> {
        ...
        // Try as a tag
        if Self::run_git_command_status(&["checkout", &format!("refs/tags/{reference}")], Some(repo_path)).is_ok() {
            return Ok(());
        }
    ...
    fn checkout_default_branch(repo_path: &Path) -> Result<()> {
        ...
        let default_branch = Self::run_git_command(&["symbolic-ref", "refs/remotes/origin/HEAD"], Some(repo_path))
            .or_else(|_| {
                // If that fails, try to set it based on the remote info
                Self::run_git_command_status(&["remote", "set-head", "origin", "--auto"], Some(repo_path)).ok();
                Self::run_git_command(&["symbolic-ref", "refs/remotes/origin/HEAD"], Some(repo_path))
            })
        ...
        if Self::run_git_command_status(&["checkout", branch_name], Some(repo_path)).is_err() {
            ...
            Self::run_git_command_status(&["checkout", "-b", branch_name, &format!("origin/{}", branch_name)], Some(repo_path))?;
        }
