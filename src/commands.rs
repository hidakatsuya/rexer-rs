...
use crate::extension::{ExtensionsConfig, LockFile, LockedExtension, Extension, ExtensionType};
    ...
    match config.load_lock_file()? {
        Some(lock_file) => {
            ...
            for ext in &lock_file.extensions {
                ...
                println!("  {} {} ({})", type_str, ext.name.blue(), ext.source.full_url());
...
pub async fn reinstall(extension_name: String) -> Result<()> {
    ...
    install_extension(&config, &Extension {
...
async fn install_environment(config: &Config, env_name: &str, extensions: &[Extension]) -> Result<()> {
    ...
    println!("Installed {} extensions for environment '{}'", extensions.len(), env_name.green());
...
async fn update_environment(_config: &Config, _extensions: &[Extension], _lock_file: &LockFile) -> Result<()> {
...
async fn run_plugin_setup(plugin_dir: &Path, config: &Config) -> Result<()> {
    ...
    if gemfile_path.exists() {
        info!("Running bundle install for plugin at {}", plugin_dir.display());
    ...
    if migrations_dir.exists() && migrations_dir.read_dir()?.next().is_some() {
        ...
        let plugin_name = plugin_dir.file_name().and_then(|n| n.to_str()).unwrap_or("unknown");
        
        run_command(
            "bundle",
            &["exec", "rake", "redmine:plugins:migrate", &format!("NAME={}", plugin_name)],
...
fn run_command(command: &str, args: &[&str], working_dir: Option<&Path>, config: &Config) -> Result<()> {
    ...
    if !status.success() {
        return Err(RexerError::GitError(format!("Command failed: {} {:?}", command, args)).into());
    }
